...
int signalProcessLC2( UInt16 *dstbuf,unsigned int *srcbuf,int size)
{
...
//	int size;
	if(alginf.command & (0x1 <<15)) {
			 mflt3x3_184((void *)dstbuf,(void*)srcbuf,184);
//			 clearfbuff((void *)0xc124000,192);
//			 alginf.nobj2=3;
		}
	else {
		if(alginf.command & (0x1 <<14)) {
				 mflt2x2_184((void *)dstbuf,(void*)srcbuf,184);
//				 alginf.nobj2=2;
			}
		else {
			if(alginf.command & (0x1 <<13))  {
				...
				if(alginf.command & (0x1 <<11))
					...
					hifr_filter184((UInt16 *) 0x812400,(void*)dstbuf,size);
//				statSMR( (UInt16 *) 0x812400, &dsmr,  &dmean);
				alginf.niter=statHistSMR184( (UInt16 *) 0x812400,  (UInt16 *) 0x824400, &dsmr,  &dmean);
//				alginf.nobj2=4;
				}
			else {
				if(alginf.command & (0x1 <<12))  {
					//diffcpy((void *)dstbuf,(void*)srcbuf,(void*) 0xc112000,192,16);
//					multpix184((void *)dstbuf,(void*)srcbuf,(void*) 0xc112000,192,16);
					//alginf.nobj2=5;
				}
				else {
					mymcpy((void *)dstbuf,(void*)srcbuf,size);
...
//		retval=m184BigFilter( (UInt16 *) 0x800400, (UInt16 *) 0x812400,  (UInt16 *) 0x824400,(UInt16 *) 0x836400, (UInt16 *) 0x83f400, (UInt8 *) 0x873600,dsmr,10*dsmr,dmean) ;
		retval=m184BigFilter( (UInt16 *) 0x800400, (UInt16 *) 0x812400,  (UInt16 *) 0x824400,(UInt16 *) 0x836400, (UInt16 *) 0x83f400, (UInt8 *) 0x873600,dsmr,10*dsmr,dmean) ;
		}
...
//	m192rnkFilter( (UInt16 *) 0x800400, (UInt16 *) 0x812400,  (UInt16 *) 0x824400,(UInt16 *) 0x836400, (UInt16 *) 0x83f400, (UInt8 *) 0x873600) ;
	if((alginf.command & (0x1 <<10))==0) {
		retval= m184rnkFilter( (UInt16 *) 0x800400, (UInt16 *) 0x812400,  (UInt16 *) 0x824400,(UInt16 *) 0x836400, (UInt16 *) 0x83f400, (UInt8 *) 0x873600,dsmr) ;
		}
	gtm=stopwatch();
...
int multpix184(UInt16* dstbuf,UInt16* sigbuf,UInt16* backbuf,int size,UInt16 zero) {
	...
	if((++pibf[0])>=184  )
		pibf[0]=0;
	for(j=0;j<size;j++) {
		dstbuf[pibf[0]*184+j]=sigbuf[96*184+96+shift];
		}
	return 1;
}
...
for(i=0;i<size;i++) {
	for(j=0;j<size;j++) {
		pfbuf[i*184+j]=pfbuf[i*184+j]*0.999 +sigbuf[i*184+j]*0.001;
		dstbuf[i*184+j]=round(pfbuf[i*184+j]);
...
int mflt2x2simple184(void * dst,void * src,int size) {
...
for(i=0;i<size-1;i++) {
	for(j=0;j<size-1;j++) {
//		dstbf[i*192+j]=(srcbf[i*192+j+1]+srcbf[(i+1)*192+j]+srcbf[(i+1)*192+j+1] +2)>>2;
		dstbf[i*184+j]=(srcbf[i*184+j]+ srcbf[i*184+j+1]+srcbf[(i+1)*184+j]+srcbf[(i+1)*184+j+1]);
		}
...
for(i=0;i<183;i++) {
	for(j=0;j<183;j++) {
//		dstbf[i*192+j]=(srcbf[i*192+j+1]+srcbf[(i+1)*192+j]+srcbf[(i+1)*192+j+1] +2)>>2;
		dstbf[i*184+j]=(srcbf[i*184+j]+ srcbf[i*184+j+1]+srcbf[(i+1)*184+j]+srcbf[(i+1)*184+j+1])>>0;
...
for(i=0,j=182;i<183;i++) {
	dstbf[(i+0)*184+j+1]=dstbf[i*184+j];
	}
for(j=0,i=182;j<184;j++) {
	dstbf[(i+1)*184+j+1]=dstbf[i*184+j];
	}
...
for(i=1;i<size-1;i++) {
	for(j=1;j<size-1;j++) {
//		dstbf[i*184+j]=(srcbf[i*184+j+1]+srcbf[(i+1)*184+j]+srcbf[(i+1)*184+j+1] +2)>>2;
		dstbf[i*184+j]=(srcbf[i*184+j]+ srcbf[i*184+j+1]+srcbf[(i+1)*184+j]+srcbf[(i+1)*184+j+1]+
						srcbf[i*184+j-1]+srcbf[(i+1)*184-j]+srcbf[(i-1)*184+j-1] + +srcbf[(i-1)*184+j+1] +srcbf[(i+1)*184+j-1])>>0;
		}
...
for(i=0,j=0;i<183;i++) {
	dstbf[(i+0)*184+j]=dstbf[i*184+j+1];
	}
for(j=0,i=0;j<184;j++) {
	dstbf[(i+0)*184+j+1]=dstbf[(i+1)*184+j];
	}
...
//	double di,dsmr,dmean;
	...
	for(i=0;i<184;i++) {
			...
			for(j=0;j<184;j++) {
				sq+=mbuff[i*184+j];
				sqsm+=mbuff[i*184+j]*mbuff[i*184+j];
				histbuf[mbuff[i*184+j]]++;
				}
			dli+=sqsm;
			}
	fMean=di=(double)sq/(184.0*184.0);
	//dsmr=sqrt((sqsm-dmn*dmn*(196.0*196.0))/(196.0*196.0));
	fSMR=sqrt(((double)dli-di*di*(184.0*184.0))/(184.0*184.0));
	ncnt=0;
	do {
//		dmean=fMean;
		dsmr=fSMR;
...
UInt32 statSMR184( UInt16 *mbuff, double *pfSMR,  double *pfMean) {
	...
	*pfMean=di=(double)sq/(184.0*184.0);
	//dsmr=sqrt((sqsm-dmn*dmn*(196.0*196.0))/(196.0*196.0));
	*pfSMR=sqrt(((double)dli-di*di*(184.0*184.0))/(184.0*184.0));
...
//	return round(2.0*dsmr2);
	...
	return round(fkSMR*MIN(dsmr2,dsmr));
//	return round(fkSMR*dsmr); //Tempporary
...
UInt32  hifr_filter184(UInt16 * mout , UInt16 * mybuff, UInt16 size) {
 int i,j;
	...
	for(i=4;i<180;i++) {
		for(j=4;j<180;j++) {
			mout[i*184+j]=512+mybuff[i*184+j]-	(( mybuff[(i-4)*184+(j-0)]+ mybuff[(i-4)*184+(j-4)]+ mybuff[(i-4)*184+(j+4)]+
			 mybuff[(i-0)*184+(j-4)]+ mybuff[(i-0)*184+(j+4)]+ mybuff[(i+4)*184+(j-0)]+ mybuff[(i+4)*184+(j-4)]+ mybuff[(i+4)*184+(j+4)])>>3);
...
UInt32 localRectStat184_(UInt16 *mbuff,double fkSMR,	UInt16 sh, UInt16 sw) {
	//double long dli;
//	double dmn2;
//	double dsmr2;
	UInt16 is,js;
...
//	sq=0; dli=0;
	...
	for(i=is-4;i<=is+4;i++) {
		for(j=js-4;j<=js+4;j++) {
//			sqsm=0;
				maxLevel=MAX(maxLevel,mbuff[i*184+j]);
				sq+=mbuff[i*184+j];
//				sqsm+=mbuff[i*192+j]*mbuff[i*192+j];
//			dli+=sqsm;
		}
...
//				maxLevel=MAX(maxLevel,mbuff[i*192+j]);
				if(mbuff[i*184+j]>maxLevel) {
					maxLevel=mbuff[i*184+j];
					im=i; jm=j;
					}
				sq+=mbuff[i*184+j];
//				sqsm+=mbuff[i*192+j]*mbuff[i*192+j];
