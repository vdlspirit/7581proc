...
int diffcpy184(UInt16* dstbuf,UInt16* sigbuf,UInt16* backbuf,int size,UInt16 zero) ;
int multpix(UInt16* dstbuf,UInt16* sigbuf,UInt16* backbuf,int size,UInt16 zero) ;
int clearfbuff184(double *pfbuf,int size) ;
...
int m184rnkFilter( UInt16 * inbuff, UInt16 * mHBuf, UInt16 * mVBuf,UInt16 * mobj,UInt16 * mobjMx,	UInt8 * mout, double dsmrpar) {
...
//	fr34j+=1.0; // Delete debug
	//fr34j+=1;
	vr5mx3mn184( (UInt16 (*)[184])  inbuff,(UInt16 (*)[184]) mVBuf,(UInt16 (*)[184]) mVBuf);
	hr5mx3mn184( (UInt16 (*)[184])  inbuff,(UInt16 (*)[184]) mHBuf,(UInt16 (*)[184]) mHBuf);
...
//				mout[i*184+j]=0;
	...
	for(i=4;i<184-4;i++) {
			for(j=4;j<184-4;j++) {
				c01=MAX(mVBuf[(i-3)*184+(j-4)],mVBuf[(i-3)*184+(j+4)]);
				c02=MAX(mHBuf[(i-4)*184+(j-3)],mHBuf[(i+4)*184+(j-3)]);
//				c01=MAX(mybuff[(i-0)*196+(j-4)],mybuff[(i+0)*196+(j+4)]);
//				c02=MAX(mybuff[(i-4)*196+(j-0)],mybuff[(i+4)*196+(j-0)]);
				c01=MAX(c01,c02);
				c00=MAX(mVBuf[(i+3)*184+(j-4)],mVBuf[(i+3)*184+(j+4)]);
				c02=MAX(mHBuf[(i-4)*184+(j+3)],mHBuf[(i+4)*184+(j+3)]);
				c00=MAX(c00,c02);
				c00=MAX(c00,c01);

				if(inbuff[i*184+j]> c00 +adds) {
							mobj[nObj]=(i<<8)+j;
//							myout[i*196+j]=1;
...
//				thr=locCombSMR(i,j,inbuff,dk);
				thr=locCombSMR184(i,j,inbuff,dk);
...
//				thr=combSMR2(i,j,inbuff,dk);
			...
			if(isSimpleThr<2) {
				...
				if(inbuff[i*184+j]>= c00+thr) {
//				mout[i*184+j]=1;
//					c02=localRectStat184((void*)0xc112000,dk2,	i, j);
					c02=localRectStat184(pbgrnd,dk2,	i, j);

//					c02=localRectStat((void*)0xc100000,dk2,	i, j);
					if(inbuff[i*184+j]>c00+c02) {
//						mout[i*184+j]=thr;

						c01=nsecCluster;
						if(mHBuf[(i-1)*184+j]>0)
						{
							n=mHBuf[(i-1)*184+j];
							...
							if(mHBuf[i*184+j-1]>0 || mHBuf[(i-1)*184+j-1]>0) { // idea - change 0 <-> 0xffff !!!!!!!!!!!!!!
								if(mHBuf[i*184+j-1]>0)
									n=mHBuf[i*184+j-1];
								else
									n=mHBuf[(i-1)*184+j-1];
								while(mVBuf[mVBuf[n]] != mVBuf[n]) {
...
//									c01=MIN(mVBuf[n],c01);
//									mVBuf[mHBuf[(i-1)*184+j+1]]=mVBuf[mHBuf[i*184+j-1]]=c01;
									}
//								else {
//									c01=MAX(mHBuf[(i-1)*184+j-1],mHBuf[(i-0)*184+j-1]);  ???????
//									}
								}
							else {
								if(mHBuf[(i-1)*184+j+1]>0) {
				...
				if(inbuff[i*184+j]> mobjMx[n]+adds) {
//				mout[i*184+j]=1;
//				c02=localRectStat184((void*)0xc112000,10.0,	i, j);
				c02=localRectStat184(pbgrnd,dk2,	i, j);

//				c02=localRectStat((void*)0xc100000,10.0,	i, j);
				if(inbuff[i*184+j]>c00+c02) {
//					smClst=mout[i*184+j-1]+mout[(i-1)*184+j-1]+mout[(i-1)*184+j+0]+mout[(i-1)*184+j+1];
//					if(smClst>0)
					mout[i*184+j]=thr;
					mobj[nObjFin]=(i<<8)+j;
					mobjMx[nObjFin]=c00;
					...
					c01=nsecCluster;
					if(mHBuf[i*184+j-1]>0) { // idea - change 0 <-> 0xffff !!!!!!!!!!!!!!
						if(mHBuf[(i-1)*184+j+1]>0) {
							c01=MIN(mHBuf[i*184+j-1],mHBuf[(i-1)*184+j+1]);
							mVBuf[mHBuf[(i-1)*184+j+1]]=mVBuf[mHBuf[i*184+j-1]]=c01;
							}
						else
							c01=mHBuf[(i-0)*184+j-1];
						}
					else {
						if(mHBuf[(i-1)*184+j+1]>0) {
							c01=mHBuf[(i-1)*184+j+1];
							}
						else {
							mVBuf[nsecCluster]=nsecCluster;
							nsecCluster++;
					...
					if(c01>nObjFin) {
						err++;
						mHBuf[i*184+j]=c00; // clear mHBuf ????
						}
					if(c01>1000) {
						err++;
						mHBuf[i*184+j]=c02; // clear mHBuf ????
						}
	...
	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mHBuf[i*184+j]=mVBuf[mHBuf[i*184+j]];
		}

	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mVBuf[nob]=mHBuf[i*184+j];
 ...
 */
	 for(n=1;n<=nCluster;n++) {
		...
		while(nob<0xffff) {
			...
			if(clst.maxl<inbuff[i*184+j])
				  clst.maxl=inbuff[i*184+j];
			clst.suml+=inbuff[i*184+j];
			mHBuf[i*184+j]=n; // Debug ????
			nob=mVBuf2[nob];
			}
...
/*
	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mVBuf[nob]=mVBuf2[mHBuf[i*184+j]];
//		mClstSz[mVBuf[nob]]++; // may be list ????
		}
...
/*
	for(i=0;i<184;i++) {
		for(j=0;j<184;j++) {
			mHBuf[i*184+j]=mVBuf2[mHBuf[i*184+j]]; // Debug ????
			}
		}
*/
	if(nCluster>1)
...
//	alginf.nobj2=nsecCluster-1;
	...
	if(clstmax.npt>0) {
		...
		for(;i<=c00;i++) {
			j=(clstmax.lt&0xff)-1;
			for(;j<=c01;j++) {
			if(inbuff[i*184+j]>clstmax.bgrnd)
				c02=inbuff[i*184+j]-clstmax.bgrnd;
				dlim+=c02*i;
				dljm+=c02*j;
				dlb+=c02;
				}
...
//		alginf.level=clstmax.maxl - clstmax.bgrnd;
		...
		alginf.rms=locCombSMR184((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0);
...
//	fr34j+=1.0; // Delete debug
	//fr34j+=1;
//	vr5mx3mn64( (UInt16 (*)[184])  inbuff,(UInt16 (*)[184]) mVBuf,(UInt16 (*)[184]) mVBuf);
//	hr5mx3mn64( (UInt16 (*)[184])  inbuff,(UInt16 (*)[184]) mHBuf,(UInt16 (*)[184]) mHBuf);
...
//					c02=localRectStat184((void*)0xc112000,dk2,	i, j);
					//c02=localRectStat184(pbgrnd,dk2,	i, j);
					c02=localBigRectStat184(pbgrnd,dk2,mean,i, j);
...
//									}
				...
				if(inbuff[i*184+j]> mobjMx[n]+adds) {
//				mout[i*184+j]=1;
//				c02=localRectStat((void*)0xc112000,10.0,	i, j);
				c02=localRectStat184(pbgrnd,10.0,	i, j);
...
//		alginf.level=clstmax.maxl - clstmax.bgrnd;
		...
		alginf.size=(((clstmax.rb>>8) - (clstmax.lt>>8)+1)<<8) | ((clstmax.rb&0xff)-(clstmax.lt&0xff));
		alginf.rms=locCombSMR184((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0); // Must Big area!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		alginf.snr=round(1000.0*(double)alginf.level/(double)alginf.rms);
		}
	else {
//		alginf.x=alginf.y=alginf.x2=alginf.y2=alginf.bgrnd=alginf.rms=0;
