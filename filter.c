...
#pragma DATA_SECTION(clstmax, ".data")
volatile CLUSTERINF clstmax;


int m192rnkFilter( UInt16 * inbuff, UInt16 * mHBuf, UInt16 * mVBuf,UInt16 * mobj,UInt16 * mobjMx,	UInt8 * mout, double dsmrpar) {
...
//	fr34j+=1.0; // Delete debug
	//fr34j+=1;
	vr5mx3mn64( (UInt16 (*)[192])  inbuff,(UInt16 (*)[192]) mVBuf,(UInt16 (*)[192]) mVBuf);
	hr5mx3mn64( (UInt16 (*)[192])  inbuff,(UInt16 (*)[192]) mHBuf,(UInt16 (*)[192]) mHBuf);
...
//				mout[i*192+j]=0;
	...
	for(i=4;i<192-4;i++) {
			for(j=4;j<192-4;j++) {
				c01=MAX(mVBuf[(i-3)*192+(j-4)],mVBuf[(i-3)*192+(j+4)]);
				c02=MAX(mHBuf[(i-4)*192+(j-3)],mHBuf[(i+4)*192+(j-3)]);
//				c01=MAX(mybuff[(i-0)*196+(j-4)],mybuff[(i+0)*196+(j+4)]);
//				c02=MAX(mybuff[(i-4)*196+(j-0)],mybuff[(i+4)*196+(j-0)]);
				c01=MAX(c01,c02);
				c00=MAX(mVBuf[(i+3)*192+(j-4)],mVBuf[(i+3)*192+(j+4)]);
				c02=MAX(mHBuf[(i-4)*192+(j+3)],mHBuf[(i+4)*192+(j+3)]);
				c00=MAX(c00,c02);
				c00=MAX(c00,c01);

				if(inbuff[i*192+j]> c00 +adds) {
							mobj[nObj]=(i<<8)+j;
//							myout[i*196+j]=1;
...
//				thr=combSMR2(i,j,inbuff,dk);
			...
			if(isSimpleThr<2) {
				...
				if(inbuff[i*192+j]>= c00+thr) {
//				mout[i*192+j]=1;
//					c02=localRectStat((void*)0xc112000,dk2,	i, j);
					c02=localRectStat(pbgrnd,dk2,	i, j);

//					c02=localRectStat((void*)0xc100000,dk2,	i, j);
					if(inbuff[i*192+j]>c00+c02) {
//						mout[i*192+j]=thr;

						c01=nsecCluster;
						if(mHBuf[(i-1)*192+j]>0)
						{
							n=mHBuf[(i-1)*192+j];
							...
							if(mHBuf[i*192+j-1]>0 || mHBuf[(i-1)*192+j-1]>0) { // idea - change 0 <-> 0xffff !!!!!!!!!!!!!!
								if(mHBuf[i*192+j-1]>0)
									n=mHBuf[i*192+j-1];
								else
									n=mHBuf[(i-1)*192+j-1];
								while(mVBuf[mVBuf[n]] != mVBuf[n]) {
...
//									c01=MIN(mVBuf[n],c01);
//									mVBuf[mHBuf[(i-1)*192+j+1]]=mVBuf[mHBuf[i*192+j-1]]=c01;
									}
//								else {
//									c01=MAX(mHBuf[(i-1)*192+j-1],mHBuf[(i-0)*192+j-1]);  ???????
//									}
								}
							else {
								if(mHBuf[(i-1)*192+j+1]>0) {
				...
				if(inbuff[i*192+j]> mobjMx[n]+adds) {
//				mout[i*192+j]=1;
//				c02=localRectStat((void*)0xc112000,10.0,	i, j);
				c02=localRectStat(pbgrnd,10.0,	i, j);
//				c02=localRectStat((void*)0xc100000,10.0,	i, j);
				if(inbuff[i*192+j]>c00+c02) {
//					smClst=mout[i*192+j-1]+mout[(i-1)*192+j-1]+mout[(i-1)*192+j+0]+mout[(i-1)*192+j+1];
//					if(smClst>0)
					mout[i*192+j]=thr;
					mobj[nObjFin]=(i<<8)+j;
					mobjMx[nObjFin]=c00;
					...
					c01=nsecCluster;
					if(mHBuf[i*192+j-1]>0) { // idea - change 0 <-> 0xffff !!!!!!!!!!!!!!
						if(mHBuf[(i-1)*192+j+1]>0) {
							c01=MIN(mHBuf[i*192+j-1],mHBuf[(i-1)*192+j+1]);
							mVBuf[mHBuf[(i-1)*192+j+1]]=mVBuf[mHBuf[i*192+j-1]]=c01;
							}
						else
							c01=mHBuf[(i-0)*192+j-1];
						}
					else {
						if(mHBuf[(i-1)*192+j+1]>0) {
							c01=mHBuf[(i-1)*192+j+1];
							}
						else {
							mVBuf[nsecCluster]=nsecCluster;
							nsecCluster++;
					...
					if(c01>nObjFin) {
						err++;
						mHBuf[i*192+j]=c00; // clear mHBuf ????
						}
					if(c01>1000) {
						err++;
						mHBuf[i*192+j]=c02; // clear mHBuf ????
						}
	...
	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mHBuf[i*192+j]=mVBuf[mHBuf[i*192+j]];
		}

	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mVBuf[nob]=mHBuf[i*192+j];
 ...
 */
	 for(n=1;n<=nCluster;n++) {
		...
		while(nob<0xffff) {
			...
			if(clst.maxl<inbuff[i*192+j])
				  clst.maxl=inbuff[i*192+j];
			clst.suml+=inbuff[i*192+j];
...
/*
	for(nob=0;nob<nObjFin;nob++) {
		i=mobj[nob]>>8;
		j=mobj[nob]&0xff;
		mVBuf[nob]=mVBuf2[mHBuf[i*192+j]];
//		mClstSz[mVBuf[nob]]++; // may be list ????
		}
...
/*
	for(i=0;i<192;i++) {
		for(j=0;j<192;j++) {
			mHBuf[i*192+j]=mVBuf2[mHBuf[i*192+j]]; // Debug ????
			}
		}
*/
	if(nCluster>1)
...
//	alginf.nobj2=nsecCluster-1;
	...
	if(clstmax.npt>0) {
		...
		for(;i<=c00;i++) {
			j=(clstmax.lt&0xff)-1;
			for(;j<=c01;j++) {
			if(inbuff[i*192+j]>clstmax.bgrnd)
				c02=inbuff[i*192+j]-clstmax.bgrnd;
				dlim+=c02*i;
				dljm+=c02*j;
				dlb+=c02;
				}
...
//		alginf.level=clstmax.maxl - clstmax.bgrnd;
		...
		alginf.rms=locCombSMR((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0);
...
//				mout[i*192+j]=1;
					//c02=1;
//					c02=localRectStat((void*)0xc112000,dk2,	i, j);
					c02=localBigRectStat(pbgrnd,dk2,mean,i, j);
...
//		alginf.level=clstmax.maxl - clstmax.bgrnd;
		...
		alginf.size=(((clstmax.rb>>8) - (clstmax.lt>>8)+1)<<8) | ((clstmax.rb&0xff)-(clstmax.lt&0xff));
//		alginf.rms=50;
//		alginf.rms=localSMR((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0);
//		alginf.rms=combSMR((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0,dsmr);
//		alginf.rms=combSMR((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0);
		alginf.rms=locCombSMR((alginf.y+5)/10,(alginf.x+5)/10,inbuff,100.0);
		alginf.snr=round(1000.0*(double)alginf.level/(double)alginf.rms);
		}
	else {
